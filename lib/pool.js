...
function process_rpc_template(rpc_template, coin, port, coinHashFactor, isHashFactorChange) {
    ...
    if (port in global.coinFuncs.getMM_PORTS()) {
        ...
        if (child_coin in activeBlockTemplates) {
            ...
            template.parent_blocktemplate_blob = global.coinFuncs
                .constructMMParentBlockBlob(
                    new Buffer(rpc_template.blocktemplate_blob, 'hex'),
                    port,
...
function templateUpdateReal(coin, port, coinHashFactor, isHashFactorChange) {
    global.coinFuncs.getPortBlockTemplate(port, function (err, body_bt) {
        if (err) {
            console.error('Block template request failed for ' + port + ' port', err);
        ...
        if (!newCoinHashFactor[coin]) {
            console.log(
                'Aborting ' + port + ' last block template request because ' + coin + ' already has zero hash factor',
            );
            return;
        }
        if (body_bt) {
                ...
                if (port in global.coinFuncs.getMM_CHILD_PORTS()) {
                    ...
                    for (let parent_port in parent_ports) {
                        ...
                        if (parent_coin in activeBlockTemplates) {
                            const parent_template = process_rpc_template(
                                activeBlockTemplates[parent_coin],
                                parent_coin,
                                parent_port,
                                lastCoinHashFactor[parent_coin],
                                false,
                            );
                            sendToWorkers({
...
function templateUpdate(coin, repeating) {
    ...
    debug(`coin=${coin}, port=${port}, coinHashFactor = ${coinHashFactor}, isMaster = ${cluster.isMaster}`);
    if (coinHashFactor) {
        global.coinFuncs.getPortLastBlockHeader(port, function (err, header) {
            if (err) {
                console.error(
                    `${threadName} Last block header request for ${port} port failed! ${JSON.stringify(err)}`,
                );
                coinHashFactorUpdate(coin, 0);
            ...
            if (!newCoinHashFactor[coin]) {
                console.log(
                    threadName +
                        'Aborting ' +
                        port +
                        ' last block header request because ' +
                        coin +
                        ' already has zero hash factor',
                );
...
function submit_block(
    ...
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [blockHex], (err, rpcResult, rpcStatus) => {
