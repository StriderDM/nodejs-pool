...
function templateUpdate(coin, repeating) {
    ...
    debug(`coin=${coin}, coinHashFactor = ${coinHashFactor}, isMaster = ${cluster.isMaster}`);
    if (coinHashFactor) {
...
function setNewCoinHashFactor(isHashFactorChange, coin, coinHashFactor, check_height) {
    ...
    if (cluster.isMaster && coin !== '' && prevCoinHashFactorMM != lastCoinHashFactorMM[coin]) {
        console.log(
            '[*] New ' +
                coin +
                ' coin hash factor is set from ' +
                prevCoinHashFactorMM +
                ' to ' +
                coinHashFactor +
                (is_mm ? ' (MM: ' + lastCoinHashFactorMM[coin] + ')' : ''),
        );
    }
...
function setNewBlockTemplate(template) {
    ...
    if (auxData) {
        ...
        consolee.warn('Not connected to merge mining proxy');
    }
    ...
    global.support.rpcDaemon('submitblock', [blockHex], (err, rpcResult, rpcStatus) => {
        ...
        if (rpcResult && rpcResult.result) {
            ...
            if (blockTemplate.isAuxChainDifficultyAchieved(TARI_COIN, hashDiff)) {
                const submitBlockAuxData = aux.findChainData(rpcResult.result, TARI_COIN);
                if (submitBlockAuxData) {
                    ...
                    global.database.insertAltBlock(templateAuxData.height, {
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    verifyShare(function (hashDiff, shareBuffer, isTrustedShare, isBlockDiffReached) {
        ...
        if (is_mm && ge(hashDiff, blockTemplate.child_template.difficulty)) {
            ...
            try {
                shareBuffer2 = global.coinFuncs.constructMMChildBlockBlob(
                    shareBuffer,
                    port,
                ...
                const err_str =
                    "Can't construct_mm_child_block_blob with " +
                    shareBuffer.toString('hex') +
                    ' parent block and ' +
                    blockTemplate.child_template_buffer.toString('hex') +
                    ' child block share buffers from ' +
                    miner.logString +
                    ': ' +
                    e;
                console.error(err_str);
                global.support.sendEmail(
                    global.config.general.adminEmail,
                    "FYI: Can't construct_mm_child_block_blob",
                    err_str,
                );
                return processShareCB(invalid_share(miner));
            }
