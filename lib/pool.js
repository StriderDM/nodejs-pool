...
let newCoinHashFactor = {}; // coin key, current individual coin hash factor, set in update
// CoinHashFactor
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        ...
        console.log(
            `>>> Trusted=${trustedShares}(${trustedSharesPercent}%) / Validated=${normalShares}(${normalSharesPercent}%) / Invalid=${invalidShares}(${invalidSharesPercent}%) / Outdated=${outdatedShares}(${outdatedSharesPercent}%) / Throttled=${throttledShares}(${throttledSharesPercent}%) / Total=${totalShares} shares`,
        );
        totalShares = 0;
        trustedShares = 0;
        normalShares = 0;
...
function registerPool() {
    global.mysql
        .query('SELECT * FROM pools WHERE id = ?', [global.config.pool_id])
        .then(function (rows) {
        ...
        .then(function () {
            global.mysql.query(
                'INSERT INTO pools (id, ip, last_checkin, active, hostname) VALUES (?, ?, now(), ?, ?) ON DUPLICATE KEY UPDATE last_checkin=now(), active=?',
            ...
            global.mysql.query('DELETE FROM ports WHERE pool_id = ?', [global.config.pool_id]).then(function () {
                global.config.ports.forEach(function (port) {
                    if ('ssl' in port && port.ssl === true) {
                        global.mysql.query(
                            'INSERT INTO ports (pool_id, network_port, starting_diff, port_type, description, hidden, ip_address, ssl_port) values (?, ?, ?, ?, ?, ?, ?, 1)',
                            [
                        ...
                        global.mysql.query(
                            'INSERT INTO ports (pool_id, network_port, starting_diff, port_type, description, hidden, ip_address, ssl_port) values (?, ?, ?, ?, ?, ?, ?, 0)',
                            [
                                global.config.pool_id,
                                port.port,
                                port.difficulty,
...
function messageHandler(message) {
    switch (message.type) {
        ...
        case 'sendRemote':
            if (cluster.isMaster) {
                global.database.sendQueue.push({
                    body: Buffer.from(message.body, 'hex'),
                });
            }
            break;
        case 'trustedShare':
...
function retargetMiners() {
    ...
    if (elapsed > 50)
        console.error(threadName + 'retargetMiners() consumed ' + elapsed + ' ms for ' + activeMiners.size + ' miners');
    process.send({
        type: 'minerPortCount',
...
function removeMiner(miner) {
    ...
    if (proxyMinerName && proxyMinerName in proxyMiners && --proxyMiners[proxyMinerName].count <= 0)
        delete proxyMiners[proxyMinerName];
...
function checkAliveMiners() {
    ...
    if (elapsed > 50)
        console.error(
            threadName + 'checkAliveMiners() consumed ' + elapsed + ' ms for ' + activeMiners.size + ' miners',
        );
}

// coin hash factor is only updated in master thread
function updateCoinHashFactor(coin) {
    debug(`updateCoinHashFactor(${coin})`);
...
function process_rpc_template(rpc_template, coin, coinHashFactor, isHashFactorChange) {
...
function templateUpdateReal(coin, coinHashFactor, isHashFactorChange) {
    global.coinFuncs.getBlockTemplate(function (err, body_bt) {
        if (err) {
            console.error('Block template request failed for ' + coin + ' port', err);
            coinHashFactorUpdate(coin, 0);
            setTimeout(templateUpdateReal, 3000, coin, coinHashFactor, isHashFactorChange);
            return;
        }
        if (!newCoinHashFactor[coin]) {
            console.log(
                'Aborting ' + coin + ' last block template request because ' + coin + ' already has zero hash factor',
            );
            return;
        }
        if (body_bt) {
            const template = process_rpc_template(body_bt, coin, coinHashFactor, isHashFactorChange);
            ...
            console.error('Block template request failed for ' + coin);
            coinHashFactorUpdate(coin, 0);
            setTimeout(templateUpdateReal, 3000, coin, coinHashFactor, isHashFactorChange);
        }
    });
...
function templateUpdate(coin, repeating) {
    ...
    debug(
        `coin=${coin}, coinHashFactor = ${coinHashFactor}, isMaster = ${
            cluster.isMaster
        } lastBlockHash = ${JSON.stringify(lastBlockHash)}`,
    );
    if (coinHashFactor) {
        global.coinFuncs.getLastBlockHeader(function (err, header) {
            if (err) {
                console.error(`${threadName} Last block header request for ${coin} failed! ${JSON.stringify(err)}`);
                coinHashFactorUpdate(coin, 0);
            ...
            if (!newCoinHashFactor[coin]) {
                console.log(
                    threadName + 'Aborting last block header request because ' + coin + ' already has zero hash factor',
                );
            ...
            if (!(coin in lastBlockHash) || header.hash !== lastBlockHash[coin]) {
                lastBlockHash[coin] = header.hash;
                templateUpdateReal(coin, coinHashFactor, isHashFactorChange);
            } else if (isHashFactorChange) {
                coinHashFactorUpdate(coin, coinHashFactor);
            }

            // TODO: #taricode
            for (let auxCoin of aux.getAuxCoinIds(header)) {
                let auxHeader = aux.findChainData(header, auxCoin).block_header;
                debug(`Got aux chain data: ${JSON.stringify(auxHeader)}`);
                if (!(auxCoin in lastBlockHash) || auxHeader.hash !== lastBlockHash[auxCoin]) {
                    debug(`New aux block detected with hash ${auxHeader.hash}`);
                    lastBlockHash[auxCoin] = auxHeader.hash;
                    templateUpdateReal(coin, coinHashFactor, isHashFactorChange);
                }
            }
...
function anchorBlockUpdate() {
    ...
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err) {
            console.error('Anchor last block header request failed!', err);
            return;
        }
        let header = body.block_header;
        anchorBlockHeight = header.height + 1;
        debug(`prev = ${anchorBlockPrevHeight} current = ${anchorBlockHeight}`);
        if (!anchorBlockPrevHeight || anchorBlockPrevHeight !== anchorBlockHeight) {
            anchorBlockPrevHeight = anchorBlockHeight;
            debug(`Anchor block was changed from ${anchorBlockPrevHeight} to  ${anchorBlockHeight}`);
        }
    });
}
...
function setNewCoinHashFactor(isHashFactorChange, coin, coinHashFactor, check_height) {
    ...
    if (cluster.isMaster && coin !== '' && prevCoinHashFactorMM != lastCoinHashFactorMM[coin]) {
        console.log(
            '[*] New ' + coin + ' coin hash factor is set from ' + prevCoinHashFactorMM + ' to ' + coinHashFactor,
        );
    }
    ...
    if (isHashFactorChange) {
        ...
        if (cluster.isMaster)
            console.log(threadName + strLogPrefix + ' with hash factor changed to ' + lastCoinHashFactorMM[coin]);
        ...
        if (cluster.isMaster)
            console.log(threadName + strLogPrefix + ' with the same ' + lastCoinHashFactorMM[coin] + ' hash factor');

        const params = getCoinJobParams(coin);
        for (let [_minerId, miner] of activeMiners) {
...
function setNewBlockTemplate(template) {
    console.log(`🧱  New block template (Height = ${template.height}, Seed = ${template.seed_hash})`);
    const auxChains = aux.getAllAuxChainData(template);
    auxChains.forEach((chain) => {
        console.log(
            `New ${chain.id} block template with (Height = ${chain.height}, Difficulty = ${chain.difficulty}, Reward = ${chain.miner_reward}, Mining Hash = ${chain.mining_hash})`,
        );
    });
    ...
    if (coin in activeBlockTemplates) {
        if (activeBlockTemplates[coin].prev_hash === template.prev_hash) {
            if ('child_template' in template) {
                if (
                    'child_template' in activeBlockTemplates[coin] &&
                    activeBlockTemplates[coin].child_template.prev_hash === template.child_template.prev_hash
                ) {
                    console.log(
                        threadName +
                            'Ignoring duplicate parent block template update at height: ' +
                            template.height +
                            '. Difficulty: ' +
                            template.difficulty,
                    );
                ...
                console.log(
                    threadName +
                        'Ignoring duplicate block template update at height: ' +
                        template.height +
                        '. Difficulty: ' +
                        template.difficulty,
                );
    ...
    if (cluster.isMaster) {
        ...
        console.log(
            '[*] New ' +
                ...
                ' difficulty and ' +
                template.port +
                ' port (with coin hash factor ' +
                template.coinHashFactor +
                ')',
        );
    } else {
        debug(
            threadName +
                'New block to mine at ' +
                template.height +
                ' height with ' +
                template.difficulty +
                ' difficulty and ' +
    ...
    const activeTemplate = (activeBlockTemplates[coin] = new global.coinFuncs.BlockTemplate(template));
    debug(`New active block template: height = ${activeTemplate.height}, difficulty = ${activeTemplate.difficulty}`);
    const auxData = activeTemplate.getAuxChainData('xtr');
    if (auxData) {
        debug(
            `New tari block template (height = ${auxData.height} difficulty = ${auxData.difficulty}, reward = ${auxData.miner_reward})`,
        );
    } else {
        console.warn('Not connected to merge mining proxy');
    }
...
function Miner(
    ...
    if (typeof login_paymentid_split[1] !== 'undefined') {
        if (
            login_paymentid_split[1].length === 64 &&
            hexMatch.test(login_paymentid_split[1]) &&
            global.coinFuncs.validatePlainAddress(this.address)
        ) {
    ...
    if (Math.abs(login_div_split.length % 2) == 0 || login_div_split.length > 5) {
        this.error =
            'Please use monero_address[.payment_id][%N%95_char_long_monero_wallet_address]+[+difficulty_number] login/user format';
        this.valid_miner = false;
        return;
    }
    ...
    for (let index = 1; index < login_div_split.length - 1; index += 2) {
        ...
        if (!global.coinFuncs.validateAddress(address)) {
            this.error =
                'Invalid payment address provided: ' +
                address +
                '. Please use 95_char_long_monero_wallet_address format';
            this.valid_miner = false;
            return;
        }
        if (address in bannedAddresses) {
            // Banned Address
    ...
    if (this.payout in bannedAddresses) {
        // Banned Address
    ...
    if (global.coinFuncs.exchangeAddresses.indexOf(this.address) !== -1 && !this.paymentID) {
        this.error =
            'Exchange addresses need 64 hex character long payment IDs. Please specify it after your wallet address as follows after dot: Wallet.PaymentID';
        this.valid_miner = false;
        return;
    }
    ...
    if (global.coinFuncs.validateAddress(this.address)) {
        ...
        global.config.general.allowBitcoin &&
        global.coinFuncs.supportsAutoExchange &&
        btcValidator.validate(this.address)
    ) {
        this.bitcoin = 1;
    } else {
        this.error =
    ...
    this.wallet_key =
        this.payout + ' ' + this.bitcoin + ' ' + this.poolTypeEnum + ' ' + JSON.stringify(this.payout_div) + ' ';
    ...
    if (email != '') {
        ...
        if (!(payoutAddress in walletLastCheckTime) || time_now - walletLastCheckTime[payoutAddress] > 60 * 1000) {
            global.mysql
                .query('SELECT id FROM users WHERE username = ? LIMIT 1', [payoutAddress])
                .then(function (rows) {
    ...
    this.storeInvalidShare = function () {
        global.database.storeInvalidShare(
            global.protos.InvalidShare.encode({
        ...
        if (this.hashes > 0) {
            debug(
                threadName +
                    ...
                    ' seconds gives: ' +
                    Math.floor(this.hashes / Math.floor((Date.now() - this.connectTime) / 1000)) +
                    ' hashes/second or: ' +
                    Math.floor(this.hashes / Math.floor((Date.now() - this.connectTime) / 1000)) *
                        global.config.pool.targetTime +
                    ' difficulty versus: ' +
                    this.newDiffToSet,
            );
        }
    ...
    this.selectBestCoin = function () {
        if (this.debugMiner) console.log(threadName + this.logString + ': current coin is ' + this.curr_coin || 'xmr');
        if (
            typeof this.curr_coin !== 'undefined' &&
            this.curr_coin_time &&
            lastCoinHashFactorMM[this.curr_coin] &&
        ...
        COINS.forEach(function (coin) {
            ...
            if (!(coin in activeBlockTemplates)) {
                if (miner.debugMiner)
                    console.log(threadName + miner.logString + ': ' + coin + ': no activeBlockTemplates');
                return;
            }
            ...
            const factor =
                (typeof miner.curr_coin_hash_factor === 'undefined' ? 1 : miner.curr_coin_hash_factor) / coinHashFactor;
            if (miner.difficulty * factor > bt.difficulty * 3) {
                debug(
                    '[MINER] Rejected best ' +
                        coin +
                        ' coin due to high diff ' +
                        miner.difficulty +
                        ' ' +
                        factor +
                        ' ' +
        ...
        if (proxyMinerName in proxyMiners) {
            ...
            if (this.debugMiner)
                console.log(
                    threadName +
                        this.logString +
                        ': calc proxy miner diff: ' +
                        miner.hashes +
                        ' / ' +
                        (Date.now() - miner.connectTime) / 1000,
                );
        } else if (
            this.payout in minerWallets &&
            minerWallets[this.payout].last_ver_shares >= MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD
        ) {
    ...
    this.checkBan = function (validShare) {
        ...
        if (validShare) {
            ...
            if (this.validShares === 0) {
                console.error(
                    threadName + 'Suspended miner IP for submitting bad share with zero trust ' + this.logString,
                );
                removeMiner(this);
        ...
        if (shareCount >= global.config.pool.banThreshold) {
            if ((100 * this.invalidShares) / shareCount >= global.config.pool.banPercent) {
                console.error(
                    threadName + 'Suspended miner IP for submitting too many bad shares recently ' + this.logString,
                );
                removeMiner(this);
                process.send({ type: 'banIP', data: this.ipAddress });
            } else {
    ...
    if (protoVersion === 1) {
        ...
        this.sendCoinJob = function (coin, params) {
            ...
            console.log(`Pushing new job to miner ${JSON.stringify(job)}`);
            if (job === null) return;
    ...
    this.toString = () =>
        `(Username = ${login}, StartingDiff = ${startingDiff}, protoVersion = ${protoVersion}, IP = ${ipAddress}, Algos = ${JSON.stringify(
            algos,
        )})`;

    console.log(`🪖  Miner login ${this.toString()}`);
}
...
function storeShare(
    ...
    if (miner.payout_div === null) {
        global.database.storeShare(
            bt_height,
            global.protos.Share.encode({
...
function recordShareData(miner, job, isTrustedShare, blockTemplate, hashDiff) {
    ...
    const wallet = walletAcc[wallet_key];
    const worker_name =
        miner.identifier in wallet || walletWorkerCount[wallet_key] < 50 ? miner.identifier : 'all_other_workers';

    if (!(worker_name in wallet)) {
        if (worker_name !== 'all_other_workers') ++walletWorkerCount[wallet_key];
        debug(
            '!!! ' + wallet_key + ': adding new worker ' + worker_name + ' (num ' + walletWorkerCount[wallet_key] + ')',
        );
        wallet[worker_name] = {};
        let worker = wallet[worker_name];
    ...
    const worker = wallet[worker_name];

    const height = worker.height;
    const difficulty = worker.difficulty;
    const acc = worker.acc;
    const acc2 = worker.acc2;
    const share_num = worker.share_num;

    console.log(blockTemplate, 'template!!');
    const altHeight = (worker.altHeight = blockTemplate.altHeight || 0);
    const shareDiff = (worker.shareDiff = hashDiff || 0);
    if (
        height !== db_job_height ||
        difficulty !== blockTemplate.difficulty ||
        time_now - worker.time > 60 * 1000 ||
        acc >= 100000000
    ) {
        if (acc != 0) {
            debug(
    ...
    debug(
        `!!!${wallet_key} / ${worker_name}: accumulating share ${db_job_height} ${blockTemplate.difficulty} ${worker.time} share: #${worker.share_num} ${worker.acc} (+${job.rewarded_difficulty} altHeight: ${worker.altHeight})`,
    );
    ...
    if (isTrustedShare) {
        ...
        debug(
            threadName +
                'Accepted trusted share at difficulty: ' +
                job.difficulty +
                '/' +
                job.rewarded_difficulty +
                ' from: ' +
                miner.logString,
        );
    } else {
        ...
        debug(
            threadName +
                'Accepted valid share at difficulty: ' +
                job.difficulty +
                '/' +
                job.rewarded_difficulty +
                ' from: ' +
...
function submit_block(
    ...
    const onError = (err, rpcResult, rpcStatus) => {
        ...
        console.error(
            threadName +
                ...
                ' (active block template height: ' +
                activeBlockTemplates[blockTemplate.coin].height +
                ') from ' +
                miner.logString +
                ', isTrustedShare: ' +
                isTrustedShare +
                    ...
                    if (blockTemplate.height === body.height + 1)
                        global.support.sendEmail(
                            ...
                            'The pool server: ' +
                                ...
                                " can't submit block to deamon on " +
                                blockTemplate.port +
                                ' port\n' +
                                'Input: ' +
                                shareBuffer.toString('hex') +
    ...
    debug(`Submitting block #${blockTemplate.height} (${blockHex}`);
    global.support.rpcDaemon('submitblock', [blockHex], (err, rpcResult, rpcStatus) => {
        // did not manage to submit a block
        if (err || !rpcResult || rpcResult.error || rpcResult.result === 'high-hash') {
            onError(err, rpcResult, rpcStatus);
        ...
        if (rpcResult && rpcResult.result) {
            ...
            if (blockTemplate.isBaseDifficultyAchieved(hashDiff)) {
                console.log(
                    `Monero difficulty achieved (height = ${blockTemplate.height}, achieved = ${hashDiff}, target = ${blockTemplate.baseCoinDifficulty}, hash = ${blockFastHash})`,
                );
                global.database.storeBlock(
                    blockTemplate.height,
                    global.protos.Block.encode({
                        hash: blockFastHash,
                        difficulty: blockTemplate.baseCoinDifficulty,
                        shares: 0,
                        timestamp: Date.now(),
                        poolType: miner.poolTypeEnum,
                        unlocked: false,
                ...
                debug(
                    `Did not achieve Monero difficulty (${blockTemplate.baseCoinDifficulty}) for block at height ${blockTemplate.height}`,
                );
            }

            const TARI_COIN = 'xtr';
            if (blockTemplate.isAuxChainDifficultyAchieved(TARI_COIN, hashDiff)) {
                const submitBlockAuxData = aux.findChainData(rpcResult, TARI_COIN);
                if (submitBlockAuxData) {
                    let templateAuxData = blockTemplate.getAuxChainData(TARI_COIN);
                    console.log(
                        `Tari difficulty achieved (height = ${templateAuxData.height}, achieved = ${hashDiff}, target = ${templateAuxData.difficulty}, hash = ${submitBlockAuxData.block_hash})`,
                    );
                    global.database.storeAltBlock(
                        templateAuxData.height,
                        global.protos.AltBlock.encode({
                            hash: submitBlockAuxData.block_hash,
                            difficulty: blockTemplate.auxDifficulty,
                            shares: 0,
                            timestamp: Date.now(),
                            poolType: miner.poolTypeEnum,
                            unlocked: false,
                            valid: true,
                            coin: 'xtr',
                            parent_chain_hash: blockFastHash,
                            port: 7878,
                            height: templateAuxData.height,
                            anchor_height: blockTemplate.height,
                            value: templateAuxData.miner_reward,
                    ...
                    console.warn(
                        'Not connected to Tari merge mining proxy. No Tari (XTR) aux data returned from submit_block. Block will be ignored',
                    );
                }
            } else {
                console.log(
                    `Did not achieve Tari difficulty (${hashDiff}) for block at base height ${blockTemplate.height}`,
                );
            }
    ...
    return (
        ...
        ((miner_wallet in walletTrust &&
            ...
            (reward_diff2 < miner_trust &&
                crypto.randomBytes(1).readUIntBE(0, 1) >
                    Math.max(256 - miner_trust / reward_diff / 2, global.config.pool.trustMin)))
    );
}
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function (verifyShareCB) {
        ...
        debug(`Verify share: achieved difficulty: ${hashDiff}, target: ${blockTemplate.difficulty}`);
        ...
        if (
                        ...
                        if (hash === null || hash === false) {
                            ...
                            console.error(
                                'EXTRA WALLET VERIFY ' +
                                    miner.payout +
                                    ': INVALID SHARE OF ' +
                                    job.rewarded_difficulty2 +
                                    ' REWARD HASHES',
                            );
                        } else {
                            extra_verify_wallet_hashes.push(
                                miner.payout +
                                    ' ' +
                                    convertedBlob.toString('hex') +
                                    ' ' +
                                    resultHash +
                                    ' ' +
                                    global.coinFuncs.algoShortTypeStr(port) +
                                    ' ' +
                                    blockTemplate.height +
                                    ' ' +
                                    blockTemplate.seed_hash,
                            );
                        }
                    });
                } else {
            ...
            if (blockTemplate.isDifficultyAchieved(hashDiff)) {
                debug(`miner:${miner.payout} Share reached target difficulty for block ${blockTemplate.height}`);
                console.log(
                    `⛏Block mined! Height = ${blockTemplate.height}, Difficulty = ${hashDiff}/${blockTemplate.difficulty}`,
                );
                console.log(miner);
                ...
                debug(
                    `Share difficulty (${hashDiff}) was below target difficulty (${blockTemplate.difficulty}} for block ${blockTemplate.height}`,
                );
                const time_now = Date.now();
                global.coinFuncs.slowHashAsync(convertedBlob, blockTemplate, function (hash) {
                    if (hash === null) {
    ...
    verifyShare(function (hashDiff, shareBuffer, isTrustedShare, isBlockDiffReached) {
        if (isBlockDiffReached === null && blockTemplate.isDifficultyAchieved(hashDiff)) {
            // Submit block to the RPC Daemon.
            if (!shareBuffer) {
                shareBuffer = getShareBuffer(miner, job, blockTemplate, params);
                if (!shareBuffer) return processShareCB(invalid_share(miner));
            }
            submit_block(miner, job, blockTemplate, shareBuffer, resultHash, isTrustedShare, true, true, hashDiff);
        }

        if (global.coinFuncs.compareDifficulty(hashDiff, job.difficulty)) {
            recordShareData(miner, job, isTrustedShare, blockTemplate, hashDiff);
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
    switch (method) {
        ...
        case 'login': {
            ...
            if (ip in bannedIPs) {
                sendReplyFinal(
                    'New connections from this IP address are temporarily suspended from mining (10 minutes max)',
                );
                return;
            }
            if (!params) {
            ...
            const miner_agent_notification =
                !global.coinFuncs.algoMainCheck(miner.algos) && global.coinFuncs.algoPrevMainCheck(miner.algos)
                    ? global.coinFuncs.get_miner_agent_warning_notification(params.agent)
                    : false;
            const miner_notification = miner_agent_notification
                ? miner_agent_notification
                : get_miner_notification(miner.payout);
            if (miner_notification) {
                if (
                    !(miner.payout in lastMinerNotifyTime) ||
                    time_now - lastMinerNotifyTime[miner.payout] > 60 * 60 * 1000
                ) {
            ...
            if (id === 'Stratum') {
                debug('Stratum detected');
                sendReply(null, 'ok');
                miner.protocol = 'grin';
            } else if (method === 'mining.authorize') {
                debug('auto mining protocol detected');
                sendReply(null, true);
                miner.protocol = 'auto';
                miner.sendBestCoinJob();
            } else {
                debug('default mining protocol detected');
        ...
        case 'getjob': {
            ...
            if (
                params.algo &&
                params.algo instanceof Array &&
                params['algo-perf'] &&
                params['algo-perf'] instanceof Object
            ) {
        ...
        case 'submit': {
            ...
            const minerId = params.id
                ? params.id
                : socket.miner_ids && socket.miner_ids.length == 1
                ? socket.miner_ids[0]
                : '';
            let miner = activeMiners.get(minerId);
            if (!miner) {
            ...
            if (miner.debugMiner) {
                console.log(`MINER DEBUG: SUBMIT ${JSON.stringify(params)}`);
            }
            miner.heartbeat();
            ...
            if (miner.proxy) {
                if (!Number.isInteger(params.poolNonce) || !Number.isInteger(params.workerNonce)) {
                    console.warn(
                        threadName + 'Malformed nonce: ' + JSON.stringify(params) + ' from ' + miner.logString,
                    );
                    miner.checkBan(false);
                    sendReply('Duplicate share');
                    miner.storeInvalidShare();
            ...
            if (nonce_test in job.submissions) {
                console.warn(
                    threadName + 'Duplicate miner share with ' + nonce_test + ' nonce from ' + miner.logString,
                );
                miner.checkBan(false);
                sendReply('Duplicate share');
                miner.storeInvalidShare();
            ...
            console.log(`🪨  processing share submission for job=${job.id} miner: ${miner.toString()}`);
...
if (global.config.general.allowStuckPoolKill && fs.existsSync('block_template_is_stuck')) {
    console.error(
        'Stuck block template was detected on previous run. Please fix monerod and remove block_template_is_stuck file after that. Exiting...',
    );
    setTimeout(function () {
        process.exit();
    }, 5 * 1000);
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        if ('' in activeBlockTemplates) {
            global.mysql.query(
                ...
                [
                    global.support.formatDate(Date.now()),
                    true,
                    activeBlockTemplates[''].height,
                    activeBlockTemplates[''].port,
                    global.config.pool_id,
                ],
            );
        } else {
            global.mysql.query('UPDATE pools SET last_checkin = ?, active = ? WHERE id = ?', [
                global.support.formatDate(Date.now()),
                true,
                global.config.pool_id,
            ]);
        ...
        global.config.ports.forEach(function (portData) {
            ...
            global.mysql.query('UPDATE ports SET lastSeen = now(), miners = ? WHERE pool_id = ? AND network_port = ?', [
                miner_count,
                global.config.pool_id,
                portData.port,
            ]);
    ...
    setInterval(function () {
        ...
        global.mysql
            .query('SELECT blockID, port FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)')
            .then(function (rows) {
                let top_height = 0;
                ...
                if (top_height) {
                    if (height < top_height - 3) {
                        ...
                        if (Date.now() - lastBlockFixTime[port] > 5 * 60 * 1000) {
                            ...
                            if (
                                lastBlockFixCount[port] > 5 &&
                                global.config.general.allowStuckPoolKill &&
                                port == global.config.daemon.port
                            ) {
                                global.support.sendEmail(
                                    global.config.general.adminEmail,
                                ...
                                console.error(
                                    'Block height was not updated for a long time for main port. Check your monerod. Exiting...',
                                );
                                fs.closeSync(fs.openSync('block_template_is_stuck', 'w'));
                                setTimeout(function () {
                                    process.exit();
                                }, 30 * 1000); // need time for admin email sending
                                return;
                            }
                        ...
                        if (height >= top_height + 3) {
                            console.warn(
                                'Current block height ' +
                                    height +
                                    ' is somehow greater than top height (' +
                                    top_height +
                                    ') amongst other leaf nodes for ' +
                                    port +
                                    ' port',
    ...
    cluster.on('exit', function (worker, code, signal) {
        ...
        global.support.sendEmail(
            global.config.general.adminEmail,
            'FYI: Started new worker ' + worker.id,
    ...
    global.support.sendEmail(
        global.config.general.adminEmail,
        'Pool server ' + global.config.hostname + ' online',
    ...
    let block_notify_server = net.createServer(function (socket) {
        ...
        socket.on('end', function () {
            ...
            console.error(`PORT2COIN[${port} == ${coin}`);
    ...
    setInterval(function () {
        ...
        fs.writeFile(fn, str, function (err) {
            if (err) console.error('Error saving ' + fn + ' file');
        });

        if (cluster.worker.id === 1) {
            let str2 = '';
            for (let agent in minerAgents) {
                str2 += agent + '\n';
            }
            const fn2 = 'miner_agents';
            fs.writeFile(fn2, str2, function (err) {
                if (err) console.error('Error saving ' + fn2 + ' file');
            });
        }
    ...
    setInterval(function () {
        ...
        fs.access(extra_wallet_verify_fn, fs.F_OK, function (err) {
            ...
            rs.on('error', function () {
                console.error("Can't open " + extra_wallet_verify_fn + ' file');
            });
            let lineReader = require('readline').createInterface({ input: rs });
            lineReader.on('line', function (line) {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, pushMessage) {
            ...
            let sendReply = function (error, result) {
                ...
                debug('[miner] reply: ' + JSON.stringify(reply));
            ...
            let sendReplyFinal = function (error) {
                setTimeout(function () {
                    ...
                    if (jsonData.id === 'Stratum') reply.method = jsonData.method;
                    debug('[miner] final reply: ' + JSON.stringify(reply));
                    socket.end(JSON.stringify(reply) + '\n');
                }, 9 * 1000);
            };
            debug('[miner] msg: ' + JSON.stringify(jsonData));
            handleMinerData(
                socket,
                jsonData.id,
                jsonData.method,
                    ...
                    if (dataBuffer.indexOf('\n') !== -1) {
                        ...
                        for (let i = 0; i < messages.length; i++) {
                            ...
                            try {
                                ...
                                let time_now = Date.now();
                                if (
                                    !(socket.remoteAddress in lastGarbageFromIpTime) ||
                                    time_now - lastGarbageFromIpTime[socket.remoteAddress] > 60 * 1000
                                ) {
                                    console.warn(
                                        threadName +
