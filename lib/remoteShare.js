    ...
    try {
        ...
        switch (msgData.msgType) {
            ...
            case global.protos.MESSAGETYPE.ALTBLOCK:
                ...
                break;
            case global.protos.MESSAGETYPE.INSERTALTBLOCK:
                try {
                    const altBlock = global.protos.AltBlock.decode(msgData.msg);
                    debug(`Inserting new alt block ${JSON.stringify(altBlock)}`);
                    global.database.insertAltBlock(msgData.exInt, altBlock, function(err){
                        if (err){
                            console.error(err);
                            return res.status(400).end();
                        } else {
                    ...
                    console.error('Bad request: Failed to decode AltBlock in /leafApi: ', e);
                    res.status(400);
                }
                break;
            case global.protos.MESSAGETYPE.INVALIDSHARE:
                debug('Storing invalid share');
