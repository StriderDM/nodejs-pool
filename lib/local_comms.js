...
function Database() {
    ...
    this.initEnv = function () {
        global.database.env = new lmdb.Env();
        ...
        global.database.syncIntervalID = setInterval(function () {
        ...
        try {
            ...
            for (let found = cursor.goToLast(), i = 0; found; found = cursor.goToPrev()) {
                ...
                cursor.getCurrentBinary(function (key, data) {
                    ...
                    let poolType = poolTypeStr(blockData.poolType);
                    if (
                        (pool_type === false || blockData.poolType === pool_type) &&
                        (!coin_port || blockData.port === coin_port)
                    ) {
                        if (typeof first !== 'undefined' && i++ < first) return;
                        response.push({
                            ts: blockData.timestamp,
    ...
    this.storeBulkShares = function (shareObject) {
        ...
        shareObject.forEach(function (share) {
            ...
            if (typeof share.raw_shares === 'number') {
                ...
                let port_suffix =
                    typeof share.port !== 'undefined' && share.port !== global.config.daemon.port
                        ? '_' + share.port.toString()
                        : '';
                    ...
                    if (cachedData.hasOwnProperty(key)) {
                        ...
                        if (cacheStore === null) {
                            ...
                            if (json_cache.hasOwnProperty('totalHashes')) {
                                // cachedData.totalHashes is always there for global and miners stats
                ...
                for (let key in shares) {
                    if (shares.hasOwnProperty(key)) {
                        ...
                        shares[key].forEach(function (final_share) {
                            ...
                            try {
                                txn.putBinary(
                                    global.database.shareDB,
                                    parseInt(key),
                                    global.protos.Share.encode(final_share),
                                );
                            } catch (e) {
                                debug(final_share);
                            }
                            if (Object.keys(shares).length === blocksSeen && sharesSeen === shares[key].length) {
                                debug('storeBulkShares: commit');
                                txn.commit();
                                return true;
                            }
                        });
    ...
    this.storeBlock = function (blockId, blockData, callback) {
        ...
        try {
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function (err, header) {
                ...
                // after 5 minutes of submit attempts finally consider this block as orphan
                if (err && header) {
                    const is_orphan =
                        (header.orphan_status && header.orphan_status === true) ||
                        (header.error &&
                            typeof header.error.message === 'string' &&
                ...
                if (err || !header) {
                    setTimeout(callback, 30 * 1000, false);
                    return;
                }
                ...
                if (txn.getBinary(global.database.blockDB, blockId) !== null) {
                    txn.abort();
                    console.error(
                        "Can't store already stored block with " +
                            blockId.toString() +
                            ' key and ' +
                            header.reward.toString() +
                            ' value: ' +
                            JSON.stringify(blockDataDecoded),
                    );
                ...
                global.database.incrementCacheData(poolTypeStr(blockDataDecoded.poolType) + '_stats2', [
                    {
    ...
    this.storeAltBlock = function (blockId, blockData, callback) {
        ...
        try {
            ...
            if (blockDataDecoded.port in busyPorts) {
                console.error(
                    'Pausing altblock with ' +
                        blockDataDecoded.port.toString() +
                        ' port and ' +
                        blockDataDecoded.height.toString() +
                        ' height processing',
                );
                setTimeout(function () {
            ...
            global.coinFuncs.getPortBlockHeaderByHash(
                blockDataDecoded.port,
                blockDataDecoded.hash,
                function (err, header) {
                    ...
                    if (err || typeof header === 'undefined' || !header) {
                        // bad block and not orphan
                        if (blockDataDecoded.hash in badBlocks) {
                            console.error(
                                'Invalidating ' + blockDataDecoded.port + ' port block hash ' + blockDataDecoded.hash,
                            );
                            return callback(true);
                        }
                        if (!(blockDataDecoded.port in potentiallyBadBlocks))
                            potentiallyBadBlocks[blockDataDecoded.port] = {};
                    ...
                    if (!is_orphan) {
                        // now we found good block (not orphan) and we can move potentiallyBadBlocks to badBlocks
                        if (blockDataDecoded.port in potentiallyBadBlocks) {
                    ...
                    let shares = global.database.getCache(
                        poolTypeStr(blockDataDecoded.poolType) + '_stats2' + port_suffix,
                    );
                    blockDataDecoded.shares = shares ? shares.roundHashes : 0;
                    ...
                    if (global.database.isAltBlockInDB(blockDataDecoded.port, blockDataDecoded.height)) {
                        console.error(
                            "Can't store already stored altblock with " +
                                blockDataDecoded.port.toString() +
                                ' port and ' +
                                blockDataDecoded.height.toString() +
                                ' height and ' +
                    ...
                    while (txn.getBinary(global.database.altblockDB, blockId) !== null) {
                        console.error(
                            "Can't store altblock with " + blockId.toString() + ' key, trying to increment it',
                        );
                        ++blockId;
                    }
                    txn.putBinary(global.database.altblockDB, blockId, blockData);
                    txn.commit();
                    global.database.incrementCacheData('global_stats2' + port_suffix, [
                        {
                            location: 'roundHashes',
                            value: false,
                        },
                    ]);
                    global.database.incrementCacheData(
                        poolTypeStr(blockDataDecoded.poolType) + '_stats2' + port_suffix,
                        [
                            {
                                location: 'roundHashes',
    ...
    this.insertAltBlock = function (blockId, block, callback) {
        try {
            const txn = global.database.env.beginTxn();
            if (txn.getBinary(global.database.altblockDB, blockId)) {
                callback(null, null);
                txn.commit();
                return;
            }
            const data = global.protos.AltBlock.encode(block);
            txn.putBinary(global.database.altblockDB, blockId, data);
            txn.commit();
            callback(null, null);
        } catch (err) {
            callback(err, null);
    ...
    this.moveAltBlockReward = function (srcBlockId, dstBlockId, srcAmount) {
        ...
        srcBlockData.pay_status =
            'Will be payed by block ' + dstBlockData.hash + ' on ' + dstBlockData.height + ' height';
    ...
    this.getValidLockedBlocks = function () {
        this.refreshEnv();
        let txn = this.env.beginTxn({ readOnly: true });
        let cursor = new lmdb.Cursor(txn, this.blockDB);
        let blockList = [];
    ...
    this.getValidLockedAltBlocks = function () {
        this.refreshEnv();
        let txn = this.env.beginTxn({ readOnly: true });
        let cursor = new lmdb.Cursor(txn, this.altblockDB);
        let blockList = [];
        for (let found = cursor.goToFirst(); found; found = cursor.goToNext()) {
            cursor.getCurrentBinary(function (key, data) {
                // jshint ignore:line
    ...
    this.cleanShareDB = function () {
        ...
        if (cleanShareInProgress) {
            ...
            if (cleanShareStuckCount > 5)
                global.support.sendEmail(global.config.general.adminEmail, 'LongRunner stuck', cleanShareStuckCount);
            ...
            [
                    ...
                    global.coinFuncs.getLastBlockHeader(function (err, body) {
                         ...
                         */
                            if (global.config.pps.enable) {
                                ...
                                callback(
                                    null,
                                    header.height - Math.floor(global.config.payout.blocksRequired / 2),
                                    Math.floor(header.difficulty * global.config.pplns.shareMulti * 2),
                                );
                            } else {
                            ...
                            if (header.height - oldestLockedBlockHeight > global.config.general.blockCleanWarning) {
                                global.support.sendEmail(
                                    global.config.general.adminEmail,
                                    'longRunner module can not clean DB good enough',
                                    'longRunner can not clean ' +
                                        (header.height - oldestLockedBlockHeight) +
                                        ' block from DB!',
                                );
                            }
                            /*
                         ...
                         */
                            if (
                                ...
                                callback(
                                    null,
                                    oldestLockedBlockHeight,
                                    Math.floor(oldestLockedBlockDifficulty * global.config.pplns.shareMulti * 2),
                                );
                            }
                        }
                    });
                ...
                function (lastBlock, difficulty, callback) {
                    ...
                    range.range(lastBlock - 1, 0, -1).forEach(function (blockID) {
                        ...
                        for (
                            let found = cursor.goToRange(parseInt(blockID)) === blockID;
                            found;
