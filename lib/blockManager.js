...
let createBlockBalanceQueue = async.queue(function (payout, callback) {
    debug(`createBlockBalanceQueue: inserting block balance ${payout.block_hash} ${payout.coin}`);
    global.mysql
    ...
    global.mysql.query('SELECT SUM(amount) as amt FROM balance where coin = ?', [coin]).then(function (rows) {
        ...
        global.mysql
            ...
            .then(function (rows) {
                if (rows.length === 0) {
                    paymentInProgress = false;
                    console.error('Error: Block ' + block_hex + ' has no block balance payments in SQL');
                    return;
                }
...
async function processPplnsPayments(block) {
    ...
    Object.keys(finalPayouts).forEach((key) => {
        ...
        let finalAmount = payout.amount / payWindow;
        debug(`${key}: ${payout.amount} / ${payWindow} = ${finalAmount}`);
