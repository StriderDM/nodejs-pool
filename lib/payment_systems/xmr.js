...
function full_stop(err) {
    ...
    global.support.sendEmail(
        ...
        'Payment daemon unable to make payment',
        'Hello,\r\nThe payment daemon has hit an issue making a payment: ' +
            JSON.stringify(err) +
    ...
    async.waterfall(
        [
            function (intCallback) {
                ...
                shapeshift.coins(function (err, coinData) {
                    if (err) {
                        ...
                        !coinData.hasOwnProperty(global.config.general.coinCode) ||
                        coinData[global.config.general.coinCode].status !== 'available'
                    ) {
                        intCallback(
                            'Coin ' + global.config.general.coinCode + ' Is not available at this time on shapeshift.',
                        );
                    } else {
                        intCallback(null);
                    }
            ...
            function (intCallback) {
                ...
                shapeshift.marketInfo(global.config.payout.shapeshiftPair, function (err, marketInfo) {
                    if (err) {
                        ...
                        !marketInfo.hasOwnProperty('limit') ||
                        marketInfo.limit <= global.support.coinToDecimal(amount)
                    ) {
                        intCallback('Not enough coin in shapeshift to process at this time.');
                    } else if (
                        !marketInfo.hasOwnProperty('min') ||
                        marketInfo.min >= global.support.coinToDecimal(amount)
                    ) {
             ...
             */
                shapeshift.shift(
                    address,
                    global.config.payout.shapeshiftPair,
                    { returnAddress: global.config.pool.address },
                    function (err, returnData) {
                        if (err) {
                            ...
                            global.mysql
                                .query(
                                    'INSERT INTO shapeshiftTxn (id, address, paymentID, depositType, withdrawl, withdrawlType, returnAddress, returnAddressType, txnStatus) VALUES (?,?,?,?,?,?,?,?,?)',
                                    [
                                        returnData.orderId,
                                        returnData.sAddress,
                                        returnData.deposit,
                                        returnData.depositType,
                                        returnData.withdrawl,
                                        returnData.withdrawlType,
                                        returnData.returnAddress,
                                        returnData.returnAddressType,
                                        'no_deposits',
                                    ],
                                )
                                .then(function () {
            ...
            function (marketInfo, shapeshiftTxnData, body, intCallback) {
                ...
                global.mysql
                    .query(
                        'INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees, exchange_rate, exchange_name, exchange_txn_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                        [
                            1,
                            address,
                            null,
                            task.amount,
                            body.tx_hash.match(hexChars)[0],
                            global.config.payout.mixIn,
                            global.support.decimalToCoin(marketInfo.minerFee),
                            1,
                            global.support.decimalToCoin(marketInfo.rate),
                            'shapeshift',
                            shapeshiftTxnData.orderId,
                        ],
                    )
        ...
        function (err, result) {
            if (err) {
                ...
                console.log(
                    'Processed ShapeShift transaction for: ' + address + ' Paid out: ' + result + ' payments in the db',
                );
    ...
    async.waterfall(
        [
                    ...
                    if (err) {
                        ...
                        console.log(
                            'Attempting to pay: ' +
                                ...
                                amtOfBTC +
                                ' BTC or ' +
                                amount / global.config.general.sigDivisor +
                                ' XMR',
                        );
            ...
            function (txnID, intCallback) {
                ...
                async.doUntil(
                    ...
                    function (xmrCallback, untilCB) {
                        return untilCB(null, xmrCallback !== 'TO_BE_CREATED');
                    },
                    function () {
            ...
            function (txnID, intCallback) {
                xmrAPIClient.post('order_status_query/', { uuid: txnID }, function (err, res, body) {
                    if (err) {
                        ...
                        global.mysql
                            .query(
                                'INSERT INTO xmrtoTxn (id, address, paymentID, depositType, withdrawl, withdrawlType, returnAddress, returnAddressType, txnStatus, amountDeposited, amountSent) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
                                [
                                    txnID,
                                    body.xmr_receiving_address,
                                    body.xmr_required_payment_id_long,
                                    'XMR',
                                    body.btc_dest_address,
                                    'BTC',
                                    global.config.pool.address,
                                    'XMR',
                                    body.state_str,
                                    global.support.decimalToCoin(body.xmr_amount_total),
                                    global.support.decimalToCoin(body.btc_amount),
                                ],
                            )
                            .then(function () {
                ...
                global.mysql
                    .query(
                        ...
                        [
                            ...
                            address,
                            null,
                            global.support.decimalToCoin(orderStatus.xmr_amount_total),
                            body.tx_hash.match(hexChars)[0],
                            global.config.payout.mixIn,
                            body.fee,
                            1,
                            global.support.decimalToCoin(orderStatus.xmr_price_btc),
                            'xmrto',
                            orderStatus.uuid,
                        ],
                    )
        ...
        function (err, result) {
            if (err) {
                ...
                // Need to fill out this data pronto!
                console.log(
                    'Processed XMRTo transaction for: ' + address + ' Paid out: ' + result + ' payments in the db',
                );
...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    function getbalance() {
        global.support.rpcWallet('getbalance', paymentDetails, function (err, body) {
            if (
                err ||
                body.hasOwnProperty('error') ||
                !body.hasOwnProperty('result') ||
                typeof body.result === 'undefined' ||
                !body.result.hasOwnProperty('unlocked_balance') ||
                typeof body.result.unlocked_balance !== 'number'
            ) {
                console.error("Can't getbalance: " + body && body.error ? JSON.stringify(body.error) : err);
            ...
            console.log(
                'Current wallet balance is ' +
                    global.support.coinToDecimal(body.result.balance) +
                    ' with ' +
                    global.support.coinToDecimal(body.result.unlocked_balance) +
                    ' unlocked balance',
            );

            let transferFunc = 'transfer';
            paymentDetails.get_tx_key = true;
            global.support.rpcWallet(transferFunc, paymentDetails, function (err, body) {
                debug('Payment made: ' + JSON.stringify(body));
                if (err || body.hasOwnProperty('error') || !body.hasOwnProperty('result')) {
                    if (err) {
                        full_stop(err);
                        return;
                    }
...
function updateShapeshiftCompletion() {
    global.mysql
        .query("SELECT * FROM shapeshiftTxn WHERE txnStatus NOT IN ('complete', 'error')")
        .then(function (rows) {
            rows.forEach(function (row) {
                shapeshift.status(row.paymentID, function (err, status, returnData) {
                    ...
                    global.mysql
                        .query('UPDATE shapeshiftTxn SET txnStatus = ? WHERE id = ?', [status, row.id])
                        .then(function () {
                            if (status === 'complete') {
                                global.mysql
                                    .query(
                                        'UPDATE shapeshiftTxn SET amountDeposited = ?, amountSent = ?, transactionHash = ? WHERE id = ?',
                                        [
                                            global.support.decimalToCoin(returnData.incomingCoin),
                                            global.support.bitcoinDecimalToCoin(returnData.outgoingCoin),
                                    ...
                                    .then(function () {
                                        global.mysql.query(
                                            'UPDATE transactions SET confirmed = 1, confirmed_time = now(), btc_amt = ? WHERE exchange_txn_id = ?',
                                            [global.support.bitcoinDecimalToCoin(returnData.outgoingCoin), row.id],
                                        );
                                    });
                                ...
                                global.mysql
                                    .query('SELECT id, xmr_amt, address FROM transactions WHERE exchange_txn_id = ?', [
                                        row.id,
                                    ])
                                    .then(function (rows) {
                                        ...
                                        global.mysql.query(
                                            'UPDATE balance SET amount = amount+? WHERE payment_address = ? limit 1',
                                            [rows[0].xmr_amt, rows[0].address],
                                        );
                                    });
...
function updateXMRToCompletion() {
    global.mysql
        .query("SELECT * FROM xmrtoTxn WHERE txnStatus NOT IN ('PAID', 'TIMED_OUT', 'NOT_FOUND', 'BTC_SENT')")
        .then(function (rows) {
            rows.forEach(function (row) {
                xmrAPIClient.post('order_status_query/', { uuid: row.id }, function (err, res, body) {
                    ...
                    global.mysql
                        .query('UPDATE xmrtoTxn SET txnStatus = ? WHERE id = ?', [body.state, row.id])
                        .then(function () {
                            if (body.status === 'BTC_SENT') {
                                global.mysql
                                    .query('UPDATE xmrtoTxn SET transactionHash = ? WHERE id = ?', [
                                        body.btc_transaction_id,
                                        row.id,
                                    ])
                                    .then(function () {
                                        global.mysql.query(
                                            'UPDATE transactions SET confirmed = 1, confirmed_time = now(), btc_amt = ? WHERE exchange_txn_id = ?',
                                            [global.support.bitcoinDecimalToCoin(body.btc_amount), row.id],
                                        );
                                    });
...
function determineBestExchange() {
    async.waterfall(
        ...
        function (err, ssValue, xmrToValue) {
            ...
            debug(
                'ShapeShift Value: ' +
                    global.support.bitcoinCoinToDecimal(ssValue) +
                    ' XMR.to Value: ' +
                    global.support.bitcoinCoinToDecimal(xmrToValue),
            );
            if (ssValue >= xmrToValue) {
                console.log(
                    'ShapeShift is the better BTC exchange, current rate: ' +
                        global.support.bitcoinCoinToDecimal(ssValue),
                );
                ...
                console.log(
                    'XMR.to is the better BTC exchange, current rate: ' +
                        global.support.bitcoinCoinToDecimal(xmrToValue),
                );
                bestExchange = 'xmrto';
...
function Payee(amount, address, paymentID, bitcoin) {
    this.amount = amount;
    this.address = aux.tryExtractAddress(address, 'xmr');
    ...
    this.setFeeAmount = function () {
        if (this.amount <= global.support.decimalToCoin(global.config.payout.walletMin)) {
            ...
            let feeValue =
                this.baseFee /
                (global.support.decimalToCoin(global.config.payout.feeSlewEnd) -
                    global.support.decimalToCoin(global.config.payout.walletMin));
            this.fee =
                this.baseFee - (this.amount - global.support.decimalToCoin(global.config.payout.walletMin)) * feeValue;
        }
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log(
                    '[*] Successful payment to ' +
                        identifier +
                        ' of ' +
                        global.support.coinToDecimal(amount) +
                        ' XMR (fee ' +
                        global.support.coinToDecimal(fee) +
                        ' - ' +
                        global.support.coinToDecimal(body.fee) +
                        ' = ' +
                        global.support.coinToDecimal(fee - body.fee) +
                        ') with tx_hash ' +
                        body.tx_hash.match(hexChars)[0] +
                        ' and tx_key ' +
                        body.tx_key,
                ...
                global.mysql
                    .query(
                        'INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                        [
                            0,
                            address,
                            paymentID,
                    ...
                    .then(function (result) {
                        if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
                            console.error(
                                "Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, '" +
                                    address +
                                    "', '" +
                                    paymentID +
                                    "', " +
    ...
    this.makePaymentAsIntegrated = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                ...
                global.mysql
                    .query(
                        'INSERT INTO transactions (bitcoin, address, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?)',
                        [0, address, amount, body.tx_hash.match(hexChars)[0], global.config.payout.mixIn, body.fee, 1],
                    )
                    .then(function (result) {
                        if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
                            console.error(
    ...
    this.manualPaymentShow = function () {
        ...
        console.error(
            '  INSERT INTO payments (unlocked_time, paid_time, pool_type, payment_address, transaction_id, bitcoin, amount, payment_id, transfer_fee) VALUES (now(), now(), ' +
                this.poolType +
                ', ' +
                this.address +
                ', ' +
    ...
    this.trackPayment = function () {
        global.mysql
            .query('UPDATE balance SET amount = amount - ? WHERE id = ?', [this.amount, this.sqlID])
            .then(function (result) {
                if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
        ...
        global.mysql
            .query('SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1', [payee.id])
            .then(function (rows) {
...
function makePayments() {
    ...
    global.mysql
        .query('SELECT * FROM balance WHERE amount >= ? and coin = "xmr"', [
            global.support.decimalToCoin(global.config.payout.walletMin),
        ])
        .then(function (rows) {
            ...
            async.eachSeries(
                ...
                function (row, next) {
                    ...
                    if (row.payment_address + (row.payment_id ? '.' + row.payment_id : '') in payeeObjects)
                        return next(); // avoid doing payment for different pool types at the same time
                    let payee = new Payee(row.amount, row.payment_address, row.payment_id, row.bitcoin);
                    global.mysql
                        .query('SELECT payout_threshold FROM users WHERE username = ?', [payee.id])
                        .then(function (userRow) {
                            ...
                            if (
                                payee.poolType === 'fees' &&
                                payee.address === global.config.payout.feeAddress &&
                                payee.amount >=
                                    global.support.decimalToCoin(global.config.payout.feesForTXN) +
                                        global.support.decimalToCoin(global.config.payout.exchangeMin)
                            ) {
                            ...
                            let remainder =
                                payee.amount % (global.config.payout.denom * global.config.general.sigDivisor);
                            if (remainder !== 0) {
                            ...
                            if (payee.amount >= threshold) {
                                ...
                                if (
                                    payee.bitcoin === 0 &&
                                    payee.paymentID === null &&
                                    payee.amount !== 0 &&
                                    payee.amount > 0 &&
                                    payee.address.length !== 106
                                ) {
                                    payeeObjects[payee.address] = payee;
                                    console.log(
                                        '[++] ' +
                                            payee.id +
                                            ' miner to bulk payment. Amount: ' +
                                            global.support.coinToDecimal(payee.amount),
                                    );
                                    paymentDestinations.push({
                                        amount: payee.amount - payee.fee,
                                        address: payee.address,
                                    });
                                    ...
                                    payee.amount > 0 &&
                                    payee.address.length === 106 &&
                                    (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) ||
                                    ...
                                    console.log(
                                        '[+] ' +
                                            payee.id +
                                            ' as separate payment to integrated address. Amount: ' +
                                            global.support.coinToDecimal(payee.amount),
                                    );
                ...
                function () {
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {
                            //jshint ignore:line
                                ...
                                paymentDetails.destinations.forEach(function (payeeItem) {
                                    ...
                                    console.log(
                                        '[**] Successful payment to ' +
                                            payeeItem.address +
                                            ' for ' +
                                            global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) +
                                            ' XMR (fee ' +
                                            global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) +
                                            ')',
                                    );
                                });
                                console.log(
                                    '[*] Successful payment to multiple miners of ' +
                                        global.support.coinToDecimal(totalAmount) +
                                        ' XMR (fee ' +
                                        global.support.coinToDecimal(totalFee) +
                                        ' - ' +
                                        global.support.coinToDecimal(body.fee) +
                                        ' = ' +
                                        global.support.coinToDecimal(totalFee - body.fee) +
                                        ') with tx_hash ' +
                                        body.tx_hash.match(hexChars)[0] +
                                        ' and tx_key ' +
                                        body.tx_key,
                                ...
                                global.mysql
                                    .query(
                                        ...
                                        [
                                            ...
                                            global.config.payout.mixIn,
                                            body.fee,
                                            paymentDetails.destinations.length,
                                        ],
                                    )
                                    .then(function (result) {
                                        if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
                                            console.error(
                                                "Can't do: INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, null, null, " +
                                                    totalAmount +
                                                    ", '" +
                                                    body.tx_hash.match(hexChars)[0] +
                                                    "', " +
...
function init() {
    ...
    setInterval(function () {
        console.log(
            'Payment queue lengths: payment (' +
                (paymentQueue.running() + paymentQueue.length()) +
                ') / shapeshift (' +
                (shapeshiftQueue.running() + shapeshiftQueue.length()) +
                ') / xmr.to (' +
                (xmrToQueue.running() + xmrToQueue.length()) +
                ')',
        );
    }, 10 * 60 * 1000);
