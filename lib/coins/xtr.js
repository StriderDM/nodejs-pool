...
function Coin(data) {
    ...
    this.testDevAddress =
        '41jrqvF7Cb7bU6SzL2pbaP4UrYTqf5wfHUqiMnNwztYg71XjbC2udj6hrN8b6npQyC2WUVFoXDViP3GFMZEYbUgR9TwJX6B'; // Address for live pool testing
    this.coinDevAddress =
        '90ea275979a642c1d84012e938c14c8068c5e1b2cf893f0dfb66a6ec07d1f169:581EfC8NL2yLZyavVrY2dGZNsqBzzcU5yE8iLrFwPLkQHxUckaWy8vjeLntrynGoBwLeKTBdgJ2rR66ZiqKogkhpS8DAMR3'; // Monero Developers Address
    this.poolDevAddress =
        '90ea275979a642c1d84012e938c14c8068c5e1b2cf893f0dfb66a6ec07d1f169:581EfC8NL2yLZyavVrY2dGZNsqBzzcU5yE8iLrFwPLkQHxUckaWy8vjeLntrynGoBwLeKTBdgJ2rR66ZiqKogkhpS8DAMR3'; // MoneroOcean Address
    ...
    this.getBlockHeaderByHash = function (blockHash, callback) {
        return this.getPortBlockHeaderByHash(global.config.daemon.port, blockHash, (err, body) => {
            if (err) {
                callback(err, null);
                return;
            }
    ...
    this.getPortBlockTemplate = function (port, callback) {
        const reserve_size = port in mm_port_set ? mm_nonce_size + pool_nonce_size : pool_nonce_size;
        const { monero: wallet_address } = aux.tryParseMergeMiningAddress(
            global.config.pool[port == global.config.daemon.port ? 'address' : 'address_' + port.toString()],
        );
    ...
    this.BlockTemplate = function (template) {
        ...
        this.baseCoinDifficulty = template.difficulty;
        let auxData = aux.getData(template);
        if (auxData) {
            debug('Received aux data in block template', template);
            this.auxChains = auxData.chains || [];
        ...
        const template_hex = template.port in mm_port_set ? extra_nonce_mm_template_hex : extra_nonce_template_hex;
        ...
        this.getAuxChainData = (coin) => {
            ...
            return this.auxChains.find((ch) => ch.id === coin);
        };
    };
    ...
    this.PORT2COIN_FULL = function (port) {
        const coin = port2coin[port];
        return !coin ? 'XMR' : coin;
    };
